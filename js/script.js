document.addEventListener('DOMContentLoaded', () => {
    ...
    function initFinalizarPedido() {
        ...
        btnWhatsapp.addEventListener('click', () => {
            if (pedidoTotal.length === 0) {
                Swal.fire({ icon: 'error', title: 'Carrinho Vazio', text: 'Você precisa adicionar pelo menos um item ao pedido antes de finalizar.', confirmButtonColor: 'var(--roxo-principal)' });
                return;
            }
            ...
            mensagem += `*TOTAL DO PEDIDO: R$ ${precoFinal.toFixed(2)}*`;
    ...
    // ===================================================================
    // FUNÇÕES AUXILIARES (COMPARTILHADAS POR TODOS)
    // ===================================================================
    ...
    function notificarSucesso(mensagem) {
        Toastify({
            text: mensagem,
            duration: 2500, gravity: "top", position: "right", stopOnFocus: true,
            style: { background: "linear-gradient(to right, #A086C4, #9175b8)" },
        }).showToast();
    }
    ...
    function injectResumoStyles() {
        ...
        style.innerHTML = `
            .item-pedido { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; margin-bottom: 12px; font-size: 0.9em; }
            .item-desc { flex: 1; padding-right: 10px; }
            .item-preco { font-weight: 600; }
            #resumo-pedido small { color: #555; line-height: 1.5; }
            .btn-remover-item { background: none; border: none; color: #cc5f5f; cursor: pointer; padding: 0 5px; font-size: 1.1em; transition: color 0.2s; }
            .btn-remover-item:hover { color: #a02c2c; }
        `;
